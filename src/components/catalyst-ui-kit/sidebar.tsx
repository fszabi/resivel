...
export function SidebarHeader({ className, ...props }: React.ComponentPropsWithoutRef<'div'>) {
  return (
    <div
      ...
      className={clsx(
        className,
        'flex flex-col border-b border-zinc-950/5 p-6 dark:border-white/5 [&>[data-slot=section]+[data-slot=section]]:mt-2.5'
      )}
    />
  )
}
...
export const SidebarItem = forwardRef(function SidebarItem(
  ...
  let classes = clsx(
    // Base
    'flex w-full items-center gap-3 rounded-lg px-2 py-2.5 text-left text-base/6 font-medium text-zinc-950 transition-all sm:py-2 sm:text-sm/5',
  ...
  return (
    <span className={clsx(className, 'relative')}>
      ...
      {'href' in props ? (
        ...
        <Headless.Button {...props} className={clsx(classes)} data-current={current ? 'true' : undefined} ref={ref}>
          <TouchTarget>{children}</TouchTarget>
        </Headless.Button>
      )}
    </span>
